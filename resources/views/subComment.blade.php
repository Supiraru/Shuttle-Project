...
@section('content')
    <div class="flex justify-center">
        <div class="w-8/12 bg-white p-6 rounded-lg">
          <p class="text-center mb-4 text-3xl font-bold text-gray-500">Sub Comment of {{ $comment->user->username }}</p>
            <div>
              ...
              <div class="flex flex-col items-end">              
                @auth
                <div class="w-4/5">
                  <form action="{{ route('SubComments.post', $comment->id) }}" method="post" class="mb-4 ">
                      @csrf
                      <div class="mb-4">
                          <label for="sub_comment" class="sr-only">Sub Comment</label>
                          <textarea name="sub_comment" id="sub_comment" cols="30" rows="4" class="bg-gray-100 border-2 w-full p-4 rounded-lg @error('sub_comment') border-red-500 @enderror" placeholder="Sub Comment something!"></textarea>
                              ...
                              <a href="">
                                <div class="flex">
                                  <div class="mr-2">
                                    <img
                                      class="rounded-full h-12 w-12"
                                      src={{ url($subComment->user->profile->avatar) }}
                                      alt="Aan"
                                    />
                                  </div>
                                  <div>
                                    <div class="flex space-x-1">
                                      <span class="font-bold">{{ $subComment->user->profile->name }}</span>
                                      <span class="text-blue-500">
                                        <svg
                                          class="w-6 h-6"
                                          fill="currentColor"
                                    ...
                                    <div class="text-gray-500 leading-4">{{ $subComment->user->username }}</div>
                                  </div>
                                </div>
                                <div class="py-3">
                                  <p class="text-lg">
                                  ...
                                  <div class="flex">
                                    <p class="text-gray-500 pt-1"> {{ $subComment->created_at->toTimeString() }} · {{ $subComment->created_at->toFormattedDateString() }}</p>
                                    <svg
                                      class="w-6 h-6 ml-auto text-gray-500"
                                      fill="currentColor"
                                      viewBox="0 0 24 24"
